...
if ($id_match > 0) {
    ...
    if (!$match) {
        $erreurs['global'] = 'Match introuvable.';
    } else {
        $adversaire   = $match['opponent'];
        $competition  = $match['competition'];
        $date_match   = $match['match_date'];
...
<?php else: ?>
    ...
    <div style="
        ...
        <form action="match_edit.php?id=<?php echo (int)$id_match; ?>" method="post"
            class="formulaire-groupe">
...
<label for="lieu">Lieu</label>
                <select id="lieu" name="lieu">
                    <option value="HOME" <?php if ($lieu === 'HOME') echo 'selected'; ?>>
                        Domicile
                    </option>
                    <option value="AWAY" <?php if ($lieu === 'AWAY') echo 'selected'; ?>>
                        Extérieur
                    </option>
                </select>
